<template>
    <div>
        <el-row class="mb20" :gutter="10">
            <el-col >
                <el-select class="wid100" v-model="selectParseUrl" placeholder="请选择">
                    <el-option
                            v-for="parseUrlVo in parseUrlVoList"
                            :key="parseUrlVo.name"
                            :label="parseUrlVo.name"
                            :value="parseUrlVo.url">
                    </el-option>
                </el-select>
            </el-col>
        ...
        <el-row class="mb20" :gutter="10">
            <el-col>
                <div class="movieListContain">
                    <el-button class="mb20 mr20 movieButton" v-for="movieVo in movieVoList" v-on:click="handleSee(movieVo.url)" >{{movieVo.num}}</el-button>
                </div>
            </el-col>
        </el-row>
        <el-row class="mb20" :gutter="10">
    ...
    export default {
        ...
        methods: {
            handleMovieList: function () {
                const that = this;
                const { url , type } = that.outerData;
                this.axios.post('/daixinmini/video/movieList', { url, type }).then(function (result) {
                    console.log(result);
            ...
            handleParseUrlList: function () {
                const that = this;
                this.axios.get('/daixinmini/videoparse/handlevideoparseurl').then(function (result) {
                    console.log(result);
                    that.parseUrlVoList = result.data;
                    that.selectParseUrl = result.data[0].url;
                });
            },
            handleSee: function (url) {
                const that = this;
                let baseUrl = that.selectParseUrl;
                console.log(baseUrl);
                that.nowSrc = baseUrl.replace(/"/g, "") + url;
                console.log(that.nowSrc);
            }
        },
        created: function () {
            const that = this;
